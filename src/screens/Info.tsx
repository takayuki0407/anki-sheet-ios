      ...
      >
        <Section title="使い方">
          <Help
            ...
            a="色付きの答え（赤やマゼンタ）が印刷されたPDFを取り込むと、答えの部分を自動で検出して隠せます。タップで答えを確認しながら暗記でき、隠し方は『赤マスク』（答えを個別に隠す）と『赤シート』（半透明のシートをスライド）から選べます。PDFの解析は端末内で完結します（解析のために送信されることはありません）。クラウド同期（Pro・Premium）を使う場合のみ、ご自身の端末間で共有するためにPDFがアカウントに保存されます。"
          />
          <Help
            q="PDFの取り込み方"
            a="本棚の「＋ 取り込む」からPDFを選びます。答えの色は『自動検出（おまかせ）』で自動判定できるほか、赤／マゼンタ／橙／青のプリセット選択や微調整も可能です。検出には少し時間がかかります（中止可）。スキャン画像のPDF（テキスト層が無いもの）は現在未対応です（今後のアップデートで対応予定）。"
          />
          <Help
            q="検出がうまくいかない時"
            a="ビューア右上の⚙設定で『自動検出』をやり直すか、答えの色プリセットや詳細（色相・許容幅・彩度の下限・見出し除外）を調整して『このPDFを再検出』してください。1ページのプレビューで結果を確認しながら調整できます。再検出しても、付けていた ★ は同じ答えに残ります。"
          />
          <Help
            q="ビューアの操作"
            a="答えをタップで表示／もう一度タップで再び隠す。隠し方は『赤マスク』（答えを個別に隠す）と『赤シート』（縦読みで半透明シートをスライド）から選べます。2本指のピンチで拡大・縮小、倍率の数字をタップで100%に戻す。『縦読み／横読み』で読み方、『目次』で見出しに移動、★で覚えにくい答えだけ復習できます。"
          />
          <Help
            q="AIで問題を作る（○×・4択）"
          ...
          <Help
            q="プランについて"
            a="サインインだけで使える Free（本1冊・AI生成 月1回）／Standard ¥300（本10冊・AI 月10回）／Pro ¥600（本 無制限・AI 月30回・全端末でクラウド同期）／Premium ¥980（Proの全機能・AI 月100回・「今日の復習」で間違えやすい問題を最適なタイミングに再出題。初回7日間無料）。AI生成は「1回＝1ページ×問題の種類」で数えます。本の冊数はアカウント全体（すべての端末の合計）で数えます。未契約でもロックされず Free として使えます。現在のプラン・AI残り枠は下の「プラン」欄で確認できます。"
          />
          <Help
            q="本のクラウド保存と復元（Pro/Premium）"
            a="Pro/Premium で取り込んだ本は PDF・検出結果・進捗（★・目次・読書位置）がクラウドに保存され、別の端末からも取り込み直せます。Standard/Free に下げると上限を超えた本はクラウドに退避し（約6ヶ月保持）、再び Pro/Premium にすると復元できます。本棚の各本の「☁️ クラウドあり／端末のみ」表示で、削除しても後で戻せるか分かります（「端末のみ」は削除すると復元できません）。"
          />
        </Section>
