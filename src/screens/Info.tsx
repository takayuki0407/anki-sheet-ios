    ...
    <View style={styles.c}>
      ...
      <ScrollView contentContainerStyle={styles.pad}>
        <Section title="使い方">
          ...
          <Help
            q="PDFの取り込み方"
            a="本棚の「＋ 取り込む」からPDFを選びます。答えの色は『自動検出（おまかせ）』で自動判定できるほか、赤／マゼンタ／橙／青のプリセット選択や微調整も可能です。検出には少し時間がかかります（中止可）。スキャン画像のPDF（テキスト層が無いもの）は現在未対応です。"
          />
          <Help
            q="検出がうまくいかない時"
          ...
          <Help
            q="AIで○×問題を作る（解いて確かめる）"
            a="本の「問題」から、ページの本文と赤シートで隠す語句をもとにAIが正誤問題（○×）を自動生成します。覚えたつもりを“解いて確かめる”ための機能です。この機能だけは、選んだページの本文と語句を当アプリのサーバー経由でAI（Anthropic）に送信します（初回に同意確認）。色の検出・赤シートなど他の機能は端末内だけで完結します。生成した問題はご自身のアカウント内に保存され、ほかのユーザーへ共有されません。プラン別に月間の生成ページ数の上限があります。"
          />
          <Help
            q="プランについて"
            a="サインインだけで使える Free（本1冊・AI 月1ページ）／Standard ¥300（本10冊・AI 月10ページ）／Pro ¥600（本 無制限・AI 月30ページ・全端末でクラウド同期）。本の冊数はアカウント全体（すべての端末の合計）で数えます。未契約でもロックされず Free として使えます。現在のプラン・AI残り枠は下の「プラン」欄で確認できます。"
          />
          <Help
            q="本のクラウド保存と復元（Pro）"
            a="Pro で取り込んだ本は PDF・検出結果・進捗（★・しおり・読書位置）がクラウドに保存され、別の端末からも取り込み直せます。Standard/Free に下げると上限を超えた本はクラウドに退避し（約6ヶ月保持）、再び Pro にすると復元できます。本棚の各本の「☁️ クラウドあり／端末のみ」表示で、削除しても後で戻せるか分かります（「端末のみ」は削除すると復元できません）。"
          />
        </Section>
