...
- [ ] **監査 E 実機検証（1.0.2）**：app.json を 1.0.2 にバンプ済（`fde9d46`・**push済**）。次 `eas build` で焼成→取り込み/閲覧の回帰確認。コードは両WebView検証済（nav guard＋onMessage origin＋originWhitelist=file://*）。OKなら universal-access 除去（同一オリジン配置改修）も検討。
